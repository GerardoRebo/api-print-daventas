...
namespace Tests\Feature\Devolucion\Data;
...
class RegisterDBData
{
    ...
    public function cargarDatos()
    {
        ...
        $this->registerArticuloAzucar();
        $this->registerArticuloKit();
    }
    ...
    function registerArticuloAzucar()
    {
        ...
        $product = new ProductArticuloVenta($this->productAzucar->id, 10, 1);
        // Create a new TicketVenta object
        $ticketVenta = new TicketVenta($ventaTicket->id);

        return $ticketVenta->registerArticulo($product);
    }
    function registerArticuloKit()
    {
        $ventaTicket = $this->ventaTicket;
        // Create a new ProductArticuloCompra object
        $product = new ProductArticuloVenta($this->productAzucarKit->id, 10, 1);
        // Create a new TicketVenta object
        $ticketVenta = new TicketVenta($ventaTicket->id);
        return $ticketVenta->registerArticulo($product);
    }
    function registerArticuloDevolucion()
    {
        $articulo = VentaticketArticulo::find(1);
        $devolucion = Devolucione::find(1);

        $devolucion->registerArticulo($articulo, 1);
    }
    function createDevolucionTicket()
    {
        $this->turno->createDevolucion($this->ventaTicket);
    }
}
