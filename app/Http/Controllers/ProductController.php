...
class ProductController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        $productosPaginados = Product::with('product_components')->leftJoin('departamento_product', 'products.id', '=', 'departamento_product.product_id')
            ...
            ->where('products.organization_id', $user->active_organization_id)
    ...
    public function searchkeyword(Request $request, ProductLogic $productLogic)
    {
        ...
        $productos = Cache::remember(
            ...
            function () use ($basicQuery, $codigo, $org) {
                return $basicQuery->where(function ($query) use ($codigo) {
                    $query->where('name', 'like', '%' . $codigo . '%')
    ...
    public function searchCode(Request $request, ProductLogic $productLogic)
    {
        $user = $request->user();
        $almacenActualId = request()->almacenActualId;
        $product = Product::where('codigo', $request->codigo)->where('organization_id', $user->active_organization_id)->first();
        if ($product == null) {
            $product = Code::where('code', $request->codigo)->where('organization_id', $user->active_organization_id)->first();
        ...
        if ($products[0]->porcentaje_ganancia == 0) {
            try {
                $tabulador =  Redis::hgetall($user->active_organization_id . "tabular");
            } catch (Exception $e) {
                return  $tabulador = [];
            }
            if (empty($tabulador)) {
    ...
    {
        //delay response for simulate scanning time
        // usleep(500000);
        $user = $request->user();
        $codigo = request()->input('codigo');
        $product = Product::where('codigo', $codigo)->where('organization_id', $user->active_organization_id)->first();
        if ($product == null) {
    ...
    public function store(Request $request, ProductLogic $pl)
    {
        ...
        $newProduct->organization_id = $user->active_organization_id;
        ...
        $precioAlmacens = [];
        $almacenes = $user->getAlmacenesByOrganization();
    ...
    public function register_service(Request $request, ProductLogic $pl)
    {
        $user = $request->user();
        $validator = FacadesValidator::make(
            $request->all(),
    ...
    public function ajustar($product, $almacenActualId, Request $request)
    {
        ...
        $precioMedioMayoreo = $request->precio_medio_mayoreo;
        ...
        $almacenActual->processCambioPrecio($user, $productA, $precioD, $precioMedioMayoreo, $precioMayoreo, true);
    ...
    public function ajustarGeneral($product, $almacenActualId, Request $request)
    {
        ...
        foreach ($user->getMyOrgAlmacens() as $almacen) {
            $almacen->processCambioPrecio($user, $productA, $precioD, $precioMedioMayoreo, $precioMayoreo, true);
        }
        $productA->procesaAjusteCosto($user, $pcosto, "Ajuste Manual");
        // if ($precioMayoreo = !0) {
        //     $precio = $productA->getPrecioModel($almacenActual->id);
    ...
    public function historialCosto(Request $request)
    {
        ...
        $history = CostoHistorial::with('user')->where('organization_id', $user->active_organization_id)
            ->where('product_id', $productActualId)
            ->whereBetween('created_at', [$dfecha, $fecha])
            ->orderBy('id', 'desc')
            ->paginate(12);
