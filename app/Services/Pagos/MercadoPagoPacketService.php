...
class MercadoPagoPacketService
{
    ...
    function createPreference(PaqueteTimbre $packet, User $user)
    {
        $intent = $packet->paymentIntent()->create([
            'user_id' => $user->id,
        ...
        $items = array(
            array(
                "title" => $packet->name,
                "quantity" => intval($packet->cantidad),
                "unit_price" => floatval($packet->price),
            )
        );
